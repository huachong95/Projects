...
private:
    ...
    const double ENC_PULSE_PER_REV = 1024.0;
    const double ENC_COUNT_PER_REV = ENC_PULSE_PER_REV * 4;
...
LogcartHardware::LogcartHardware(ros::NodeHandle &nh)
    : nh(nh), baud_rate(9600), isSerialPortDown(false), serialStrBuffer("")
{
...
void LogcartHardware::processHeartbeatMessage(std::string &serialStr, int indexHeader)
{
    ...
    if(serialStr.find("+") != std::string::npos)
    {
        ...
        if(std::count(strHeartBeat.begin(), strHeartBeat.end(), '_') == (FIELDS_IN_HEARTBEAT_MSG - 1))
        {
            ...
            char* strUltrasonic1    = strtok(NULL, "_");
            char* strUltrasonic2    = strtok(NULL, "_");
            char* strUltrasonic3    = strtok(NULL, "_");
            char* strUltrasonic4    = strtok(NULL, "_");
            ROS_DEBUG("Payload Strings: %s, %s, %s, %s, %s, %s, %s, %s", strSpeed1, strSpeed2, strEncoder1, strEncoder2, strUltrasonic1, strUltrasonic2, strUltrasonic3, strUltrasonic4);
            ...
            int ultrasonic1 = atoi((isValidNumber(strUltrasonic1)   ? strUltrasonic1 : "0"));
            int ultrasonic2 = atoi((isValidNumber(strUltrasonic2)   ? strUltrasonic2 : "0"));
            int ultrasonic3 = atoi((isValidNumber(strUltrasonic3)   ? strUltrasonic3 : "0"));
            int ultrasonic4 = atoi((isValidNumber(strUltrasonic4)   ? strUltrasonic4 : "0"));
            ROS_DEBUG("Payload Values: %d, %d, %ld, %ld, %d, %d, %d, %d", speed1, speed2, encoder1, encoder2, ultrasonic1, ultrasonic2, ultrasonic3, ultrasonic4);
